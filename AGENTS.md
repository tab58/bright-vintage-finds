...
│       ├── src/pages/           # Home (public shop front), Inventory (status-grouped list, long-press moves), Intake (photos + all fields → draft), Item (read-only detail + Edit-details toggle; places/labels/Whatnot/notes always editable; status actions, mark-sold dialog)
