...
│   └── public_site/             # Splash page (/) + inventory PWA (/inventory*), Vite + React + TS + Tailwind v4 + shadcn/ui
│       ├── src/App.tsx          # Router: splash at /, inventory pages under /inventory
│       ├── src/pages/           # Inventory (status-grouped list, long-press moves), Intake (photos + all fields → draft), Item (read-only detail + Edit-details toggle; places/labels/Whatnot/notes always editable; status actions, mark-sold dialog)
│       ├── src/components/      # Shared UI: item-fields.tsx (the item form, used by intake + item detail), chip-picker.tsx
...
│       └── vite.config.ts       # Tailwind v4 plugin + PWA (install-only) + '@' → /src alias + dev proxy /admin → localhost:3000
└── environment/
...
**Frontend:** Vite 5, React 18, TypeScript, Tailwind CSS v4 (via `@tailwindcss/vite`), shadcn/ui components (`npx shadcn@latest add <name>`), lucide-react icons. Commands (from `frontend/public_site/`): `npm run dev` / `npm run build` / `npm run preview`.
