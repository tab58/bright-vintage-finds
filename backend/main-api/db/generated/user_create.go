...
func (_c *UserCreate) check() error {
	...
	if _, ok := _c.mutation.Version(); !ok {
		return &ValidationError{Name: "version", err: errors.New(`generated: missing required field "User.version"`)}
	}
	if _, ok := _c.mutation.IdpID(); !ok {
		return &ValidationError{Name: "idp_id", err: errors.New(`generated: missing required field "User.idp_id"`)}
	}
	...
	if _, ok := _c.mutation.FullName(); !ok {
		return &ValidationError{Name: "full_name", err: errors.New(`generated: missing required field "User.full_name"`)}
	}
	if v, ok := _c.mutation.FullName(); ok {
		if err := user.FullNameValidator(v); err != nil {
			return &ValidationError{Name: "full_name", err: fmt.Errorf(`generated: validator failed for field "User.full_name": %w`, err)}
		}
	}
	if _, ok := _c.mutation.AccountStatus(); !ok {
